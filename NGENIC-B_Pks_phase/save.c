...
#define BUFFER 10
  ...
  mkdir(OutputDir, 0777); //Create folder if it does not already exist
...
if (NU_On==1){ //Only run this part if initial glassfile has neutrinos and so we need to remove them. That is for 3 mesh.
  int NumPartold=NumPart; //THis is the number of particles in the working cpu
  int TotNumPartold=TotNumPart; //This is the number of particles in overall
  NumPart=NumPartold*2/3; //I am decreasing numpart to not count neutrinos, so that neutrinos are not looped over.
  TotNumPart=TotNumPartold*2/3;
  OmegaDM_2ndSpecies=0;
  i_fin=2;
}

  printf("\n header1.npartTotal[0]=%d; header1.npartTotal[1]=%d; header1.npartTotal[2]=%d; \n",header1.npartTotal[1],header1.npartTotal[2],header1.npartTotal[3]);

  for(i = 0; i < i_fin; i++)/*only loop over baryons and DM. TO loop over neutrinos set i_fin=3*/
    header.npartTotal[i] = header1.npartTotal[i + 1] * GlassTileFac * GlassTileFac * GlassTileFac;

  printf("\n header.npartTotal[0]=%d; header.npartTotal[1]=%d; header.npartTotal[2]=%d; \n",header.npartTotal[0],header.npartTotal[1],header.npartTotal[2]);
  ...
  if(header.npartTotal[1])
    header.mass[1] =
      (Omega - OmegaBaryon - OmegaDM_2ndSpecies) * 3 * Hubble * Hubble / (8 * PI * G) * pow(Box,3)/(header.npartTotal[1]);
