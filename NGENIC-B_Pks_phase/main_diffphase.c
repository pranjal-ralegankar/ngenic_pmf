...
/*****************************providing parameters to compute magnetic fields*********************************/
  ...
  Amp=2*PI*PI*pow(10,-4)*pow(B1mpc,4)*GnB*pow(0.678,3)*3.91*pow(10,6);//2.15*pow(10,-10)*4*PI*PI/19.47*pow(B1mpc,2)/kmpc/kmpc/kmpc;
		 ...
		 (ii >= Local_x_start && ii < (Local_x_start + Local_nx)))
		{
		  ...
		  for(j = 0; j < Nmesh; j++)
			...
			//    printf("k=%g; pk=%g; k2=%g; pk2=%g; type=%d\n",0.054*kmpc,PowerSpec(0.054*kmpc),0.844*kmpc,PowerSpec(0.844*kmpc),Type);	
			  ...
			  pkb=Amp*pow(lambda_pmf,2)*pow(kmag/kmpc,2*nB+7)*exp(-2*kmag2/kd/kd);
			  pkb *= -log(ampl2);
